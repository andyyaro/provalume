...
| `reverification.executed` | outcome `passed` → discharges exactly the triggers in `answers_triggers`; `current` only when none remain outstanding; `failed` → `stale`; `errored` → **no transition** (fail-open) | `record_id`, `trigger_commit`, `command` (the **raw** command as verified), `exit_code`, `duration_ms`, `timeout_ms` (the configured bound — a timeout kill must be distinguishable from an ordinary failure), `environment_fingerprint` (computed **before** the run), `outcome` (`passed` \| `failed` \| `errored`), `executed_at_commit` (the tree's HEAD), `answers_triggers` (outstanding triggers that are ancestors of that HEAD — a pass at an older or detached HEAD discharges only what it answered), `root`, `signal` (present when a signal death forced `errored`) |
