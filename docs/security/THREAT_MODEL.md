...
| ID | Threat | Control | Severity |
|---|---|---|---|
| **T27** | **Stored-command re-execution as a code-execution path.** A poisoned proposal, a tampered record, or simply a careless original command becomes something Provalume itself executes later, unattended. This is the closest thing to remote code execution this design has ever contained. | Re-execution is **off by default**: the command allowlist ships empty, and an empty allowlist disables the feature entirely. Enabling it is an explicit per-repository operator action, and the allowlist, the execution root, and the repository identity all anchor to the **database's** repository — a record refuses to execute in a tree whose `repository_id` differs from its own, so standing in the wrong directory cannot make one project's allowlist enable another project's commands. Only `verified`-or-above **procedural records whose verification passed** are eligible — agent-sourced records are capped at `observed`; gotchas refuse because their claim IS the failure and the outcome mapping would invert. What executes is the record's **raw** command exactly as verified (never the normalized rewrite, never anything caller-supplied), as an argument vector, never `shell=True`, under a hard timeout, only in a worktree **clean of tracked modifications** — uncommitted state can neither trigger freshness nor clear it. A passing run discharges only the triggers whose commits are ancestors of the HEAD it ran at. A **negative exit (signal death: OOM killer, SIGTERM) journals `errored`**, never `failed` — the environment killing the process is not evidence against the claim. There is no daemon: execution happens only inside an explicit CLI invocation or an operator-installed hook. Every execution is journaled with command, exit code, duration, the configured timeout bound, the HEAD it ran at, the triggers it answers, its root, and a **pre-run** environment fingerprint; if journaling itself fails after a run, the executor logs the execution loudly rather than letting it vanish. Allowlist patterns are `fnmatch`, case-sensitive on every platform; `*` spans spaces, so operators must anchor patterns at the start of the command — a leading `*` does not constrain which program runs. | Critical |
| **T28** | **Freshness suppression as a trust-erosion attack.** An attacker who can land commits touches files inside a true record's blast radius so the record flips to `suspect` (or engineers a failing re-run to reach `stale`), degrading confidence in true facts — potentially so a poisoned alternative outranks them. | Only **landed** commits trigger freshness transitions — the same bar semantic truth itself requires, so the attacker needs the same access that already lets them change project truth. `suspect` and `stale` relabel and demote; they never remove a record from retrieval and never grant trust to anything else. `stale` additionally requires an actual failed re-execution under the T27 control set, with the environment fingerprint recorded. Only kernel-sourced freshness events participate in derivation — an imported or agent-sourced `freshness.triggered` is stored and derives nothing. Trust invalidation — whether by reviewer judgement or the existing `invalidate.commit_reverted` rule — is a trust transition this machinery cannot produce. The full trigger → assessment → execution chain is appended to the journal and auditable. | Medium |
