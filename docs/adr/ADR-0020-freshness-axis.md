...
| `reverification.executed` (outcome `passed`) | discharges exactly the triggers named in `answers_triggers` — the outstanding triggers whose commits are ancestors of the HEAD the run executed at — and the record returns to `current` only when none remain outstanding. A pass at an older or detached HEAD answers only what it can see — and a trigger whose commit was rewritten out of history (reset, amend, squash) is never an ancestor again, so only a fresh blast-radius measurement discharges it (LIMITATIONS §9g); that measurement still discharges everything, because it re-measures against the tree as it stands |
