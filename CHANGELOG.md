...
**The relevance filter.** Triggers are assessed in the same scan by a
deterministic stdlib-`ast`/`tokenize` differ — never a model call — with a
closed set of reason codes. Whitespace-, comment-, and docstring-only
landings discharge their own trigger and leave the record `current`;
signature, import, and body changes keep it `suspect`; a file the differ
cannot parse escalates, because a differ that cannot read a change does not
get to call it harmless. Records verified by comment-reading commands
(linters, type checkers, doctest runners) get no trivia short-circuit at
all — for them a `# noqa` IS the outcome (LIMITATIONS §9e). A change git
reports but the text layer cannot see (a mode flip, an eol conversion)
escalates rather than passing as whitespace. Bookkeeping is per trigger
commit: a verdict answers exactly the landing it assessed, an assessment
that fails open is counted and surfaced (never silently unretryable — a
re-scan re-assesses it), and assessment is bounded per record
(LIMITATIONS §9f). The differ's blind spots are chosen in the escalating
direction: token-level classifications are cross-checked against the AST,
and `differ_version` rides on every verdict so a future precision
